...
Completa en `.env` las credenciales públicas de Supabase. La aplicación requiere una sesión autenticada por OTP de correo; no agregues claves de OpenAI, Resend, SMTP ni `service_role` a este archivo.

## Acceso por correo OTP
...
4. En **Authentication → Email Templates**, modifica el correo Magic Link para incluir `{{ .Token }}`; esto envía el código de seis dígitos que usa la pantalla `/login`.
5. Configura las URLs de desarrollo y producción en **Authentication → URL Configuration**.
