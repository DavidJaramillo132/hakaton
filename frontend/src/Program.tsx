...
=======
  const [panel, setPanel] = useState<Panel>("form"); const [error, setError] = useState<string | null>(null); const [result, setResult] = useState<ResultState | null>(null); const [historyOpen, setHistoryOpen] = useState(false); const [historyVersion, setHistoryVersion] = useState(0);
>>>>>>> Stashed changes
  const analyze = async () => {
    ...
    try {
      ...
      setResult({ field, response }); setHistoryVersion((version) => version + 1); setPanel("result");
...
=======
      ...
      </div></main><button type="button" aria-label={historyOpen ? "Cerrar historial" : "Ver historial"} aria-expanded={historyOpen} onClick={() => setHistoryOpen((open) => !open)} className="fixed right-0 top-1/2 z-[900] -translate-y-1/2 rounded-l-xl bg-leaf-700 px-2 py-4 text-xs font-bold text-white shadow-lg transition hover:bg-leaf-800 focus:outline-none focus:ring-4 focus:ring-leaf-200"><span className="[writing-mode:vertical-rl]">{historyOpen ? "Cerrar historial" : "Ver historial"}</span></button><HistoryDrawer open={historyOpen} onClose={() => setHistoryOpen(false)} refreshToken={historyVersion} /></div>;
>>>>>>> Stashed changes
}
