...
export function App() {
  const [user, setUser] = useState<User | null>(null); const [sessionError, setSessionError] = useState<string | null>(null); const [booting, setBooting] = useState(true);
  ...
  const startSession = useCallback(async () => { setBooting(true); setSessionError(null); try { setUser(await ensureAnonymousSession()); } catch (issue) { setSessionError(errorText(issue)); } finally { setBooting(false); } }, []);
  ...
  return <div className="min-h-screen bg-[#f5f7f2] text-slate-800"><header className="border-b border-leaf-100 bg-white"><div className="mx-auto flex max-w-[1540px] items-center justify-between px-5 py-4 lg:px-8"><a href="/" className="flex items-center gap-2 font-display text-xl font-bold text-leaf-900"><span className="grid h-9 w-9 place-items-center rounded-xl bg-leaf-600 text-xl text-white">⌁</span>AgroClima <span className="text-amber-600">IA</span></a><p className="flex items-center gap-2 text-xs font-medium text-slate-500"><span className={`h-2 w-2 rounded-full ${user ? "bg-emerald-500" : "bg-amber-400"}`} />{booting ? "Conectando…" : user ? "Sesión protegida" : "Sin conexión"}</p></div></header>
    <main className="mx-auto max-w-[1540px] px-5 py-7 lg:px-8 lg:py-10"><div className="mb-6"><p className="text-xs font-bold uppercase tracking-[.18em] text-leaf-700">Portoviejo · Manabí</p><h1 className="mt-2 font-display text-3xl font-bold tracking-tight text-leaf-900 sm:text-4xl">Protege tu cultivo antes de que cambie el clima.</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600">Dibuja la parcela y recibe precauciones climáticas específicas para tu cultivo.</p></div>
      {!isSupabaseConfigured && <Notice type="warning">Configura <code>VITE_SUPABASE_URL</code> y <code>VITE_SUPABASE_ANON_KEY</code> en <code>.env</code> para habilitar el análisis.</Notice>}{sessionError && <Notice type="error">{sessionError} <button className="ml-2 font-bold underline" onClick={() => void startSession()}>Reintentar</button></Notice>}{error && <Notice type="error">{error}<button onClick={() => setError(null)} className="ml-auto px-1 text-lg" aria-label="Cerrar error">×</button></Notice>}
