...
export function RiskResult({ campo, response, climate, onNew }: Props) {
  ...
  return <section aria-live="polite" className="space-y-4">
    ...
    <button onClick={onNew} className="w-full rounded-xl border border-slate-300 bg-white px-4 py-3 text-sm font-bold text-slate-700 transition hover:bg-slate-50">← Analizar otro campo</button>
  </section>;
}
